...
export default class NewsService {
  public static getAllNews(publishedOnly?: boolean) {
    const newsArray = publishedOnly ? Storage.publishedNews : Storage.news;
  ...
  public static getNewsFilteredPaginated(
    req: GetNewsRequest,
    authData: AuthData
  ) {
    ...
    if (req.tags || req.noTagsNoNews) {
      news = news.filter((item) =>
        (req.tags || '').split(',').find((tag) => item.tags.has(tag))
      );
    }
    if (req.onlyNew === 'true') {
      const user = Storage.users.get(authData.userId);
    ...
    if (req.author) {
      news = news.filter((item) => item.author === Number(req.author));
    }
    if (req.header) {
  ...
  public static createNews(req: UpdateNewsRequest, auth: AuthData) {
    const news = new News({
      ...
      publicationDate:
        new Date(req.publicationDate) ??
        (req.state === State.published && new Date()),
    });
  ...
  public static updateNews(id: number, req: UpdateNewsRequest, auth: AuthData) {
    ...
    oldNews.publicationDate =
      new Date(req.publicationDate) ??
      (oldNews.state === State.draft &&
        req.state === State.published &&
        new Date());
